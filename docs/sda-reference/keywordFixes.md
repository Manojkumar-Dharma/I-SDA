...
| [I-107](#i-107) | Record | `CHECK(AB)` / `CHECK(RL)`: not guarded on `MNUBAR` / `USRDFN` records | I-104 | In progress | — |
...
|-------|------|--------|-------|
| 1 | [I-107](#i-107) | In progress | `CHECK(AB)` / `CHECK(RL)`: not guarded on `MNUBAR` / `USRDFN` records. Size (estimate): Small. Raised by I-104. |
...
- *Record-level-only, to read (31):* `ALWROL`, `ASSUME`, `CLRL`, `CSRLOC`, `GETRETAIN`, `GRDRCD`, `HLPCMDKEY`, `HLPSEQ`, `INZRCD`, `LOGINP`, `MNUBAR`, `PULLDOWN`, `RTNCSRLOC`, `RTNDTA`, `SETOF`, `SFL`, `SFLCTL`, `SFLENTER`, `SFLLIN`, `SFLMLTCHC`, `SFLMODE`, `SFLMSGRCD`, `SFLPAG`, `SFLRNA`, `SFLRTNSEL`, `SFLSIZ`, `SFLSNGCHC`, `SLNO`, `UNLOCK`, `USRDFN`, `WINDOW`. Watch for: `ALWROL` (other keywords *require* indicators when combined with it), `CSRLOC`, `SETOF`, `SFLMODE`, `HLPCMDKEY` (their sections also contain a "valid" sentence), and `SFLSIZ`/`SFLPAG`/`SFLLIN`/`SFLMSGRCD` (display-size conditions, like `MSGLOC`). `LOGINP` and `CHECK(AB)/(RL)` are already right in the structured panels (I-9, I-98).
...
Suggested order: the record-level-only batch next (the most keywords whose rule is stated plainly), then the field-level-only batch, then the multi-level ones last, since they need a design decision.
