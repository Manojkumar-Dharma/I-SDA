...
- *Record-level-only - done (batch 2, plus `SFLMODE`) except `CSRLOC`,* which says "valid" and is correctly not a table entry. 30 of the original 31 are in the table.
- *Field-level-only - done in batch 3 except two:* `MSGCON` (valid for the message presence) and `MSGID` (conditional, I-73). The other 35 of the original 37 are in the table.
- *Help-specification level:* `HLPARA`.
- *Multi-level keywords - read after batch 3, and most are simpler than expected:* a name-keyed entry is right whenever the reference gives the **same** rule at every level the keyword exists at. That holds for `CHANGE` (record or field), `CHGINPDFT` (file, record, field), `INDTXT` (file, record, field), `TEXT` (record or field), `VALNUM` and `WRDWRAP` (file, record, field) and `VLDCMDKEY` (file or record): each says a plain "Option indicators are not valid for this keyword". Four keywords the earlier scan put here are in fact field-level only with a plain sentence - `REFFLD`, `SFLRCDNBR`, `SFLROLVAL`, `SFLSCROLL` (the last three on the subfile-control record) - and `ALTNAME` is record-level with the sentence in the System/36 chapter ("not valid for this keyword", "not allowed with this keyword"). None of these has been added yet (structured panels still to be checked). **Genuinely level-dependent:** `HLPTITLE` (not valid on a file-level HLPTITLE, allowed on record-level ones, up to 15 with indicators). **Conditional per parameter, already covered by their own checks:** `CHECK` (I-30: indicators valid only for `CHECK(ER)`/`CHECK(ME)`) and `MSGID` (I-73). **Different rule, needs a decision:** `SFLMSGKEY` ("not valid for this keyword *or with the associated field*") and `SFLPGMQ` ("option indicators *and display size condition names* are not valid").
- *In I-95's list but not confirmed - read directly:* `CHOICE`, `GRDCLR`, `HOME` and `WDWTITLE` each say "Option indicators are **valid** for this keyword" - not candidates, nothing to add. `ALTNAME`: see above.

What is left after batch 3: the multi-level group above as a batch 4 (12 name-keyed entries, after checking their structured panels), and three small decisions - `HLPTITLE` (level-aware raw editor or leave), `SFLMSGKEY` (also guard conditioning of its field?) and `SFLPGMQ` (mention display size conditions too?). `CSRLOC`, `MSGCON`, `MSGID`, `CHECK`, `CHOICE`, `GRDCLR`, `HOME` and `WDWTITLE` need no table entry.
