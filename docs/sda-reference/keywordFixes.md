...
| **I-52** | Gap found while implementing I-45: `DSPMOD`'s own DDS Reference text has a SECOND, independent prerequisite beyond the `DSPSIZ` one I-45 fixed - "The DSPMOD keyword cannot be specified on a subfile record (SFL keyword)." Out of I-45's own stated scope (that task's own row named only the `DSPSIZ` prerequisite), so not fixed there. Distinct from I-46 (currently claimed/in progress): I-46 is re-reading `SFL`/`SFLCTL`'s OWN DDS Reference sections for a USRDFN-style blanket rule on what else can coexist on an SFL/SFLCTL record; this finding is the mirror case - a restriction stated in `DSPMOD`'s OWN section, naming SFL specifically - and may or may not already surface once I-46 lands, depending on what that investigation finds. Scope: confirm whether `DSPMOD`'s row can currently be turned on for an SFL/SFLCTL record at all (`isSflRecord`/`isSflCtlRecord` category-gating may already close this the same way Task R2 closes 25 of I-44's own keywords for USRDFN - check before assuming a live gap), and if reachable, add a guard (same alert+revert idiom) analogous to I-45's own `dspmodDspsizPrerequisiteReason`. Not yet fixed, logged for later pickup. | I-45 (found during its implementation) | not started |
