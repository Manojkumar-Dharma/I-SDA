...
> **Area:** Tooling / all levels (file, record, field, help entry) · **Status:** In progress · **Depends on:** I-95
...
**Batch 2 - the record-level-only keywords (v0.10.195).** Done: 29 names added to `NO_OPTION_INDICATOR_KEYWORDS` (now 44 entries with batch 1 and `IGCALTTYP`). Each was read in `DDS_Keyword_V7r6.txt`: every section of it that describes a level says "record-level keyword" (the other hits are table-of-contents and index fragments), and says plainly "Option indicators are not valid for this keyword", with no "valid" sentence.
...
- **Held back, on purpose:** `CSRLOC` (its section says option indicators **are** valid - I-21 already gives it a Conditioning toggle) and `SFLMODE` (its section carries both a "not valid" and a "valid" sentence, so it needs its own read). Neither is in the table.
...
- **Test.** New `src/test/i101RecordLevelNoOptionIndicators.test.js` (435 checks, wired into `npm test`): the table (44 entries, the diff check, `CSRLOC`/`SFLMODE`/`TEXT`/`CHECK`/`KEEP`/`HELP` still `null`); **every entry checked against the reference text itself** (record-level section, the plain sentence, no "valid" sentence, display size mentioned exactly for the five); all 29 in the raw editor with no condition (no toggle, a note), hand-written with an indicator (toggle kept, a warning naming the keyword, adding refused, removing allowed), the five with a `*DS4` condition (toggle kept, an option indicator on top refused), `CSRLOC` still takes one; the structured-panel pin; and the real generated designer's record raw editor (an `SFLCTL` record with `SFLSIZ`, `SFLPAG` with an indicator and under `*DS4`, `CSRLOC`, and an `INZRCD` record). 186 checks fail against the pre-fix code. `i101FileLevelNoOptionIndicators.test.js`'s count check was relaxed from "15 entries" to "at least those 15".

**What is left (the audit continues; this task stays In progress).** Re-scanning the reference for keyword sections with an "Option indicators are not valid/allowed" sentence found **98** keywords, against I-95's 92 named ones (its "93" count and its two lists do not quite agree either). The sets differ by name, and most of the difference is the reference itself: several sections exist twice (the System/36 environment chapter repeats some keywords' text, e.g. `USRDSPMGT` and `ALTNAME`'s statement live *only* there), and a scan that stops at a section's own heading misses them. **Treat the lists below as a reading queue, not as verified entries** - each keyword still has to be read before it is added, and the "conditional" ones need their own check rather than a table entry.

- *Record-level-only - done in batch 2 except two:* `CSRLOC` (says "valid" - not a table entry) and `SFLMODE` (both sentences - read directly before deciding). The other 29 of the original 31 are in the table.
- *Field-level-only, to read (37 + `IGCALTTYP`, done in I-95):* `ALIAS`, `BLANKS`, `BLKFOLD`, `CHCACCEL`, `CHCCTL`, `CHKMSGID`, `CHRID`, `CNTFLD`, `COMP`, `DATE`, `DATFMT`, `DATSEP`, `DFT`, `DLTCHK`, `DLTEDT`, `EDTCDE`, `EDTMSK`, `EDTWRD`, `FLDCSRPRG`, `FLTFIXDEC`, `HLPID`, `HTML`, `MAPVAL`, `MLTCHCFLD`, `MSGCON`, `MSGID` (conditional - I-73), `PSHBTNFLD`, `RANGE`, `SFLCHCCTL`, `SFLCSRPRG`, `SNGCHCFLD`, `SYSNAME`, `TIME`, `TIMFMT`, `TIMSEP`, `USER`, `VALUES`. Watch for `CHCACCEL`, `CHCCTL`, `COMP`, `DATE`/`DATFMT`/`DATSEP`, `DFT`, `HTML`, `SYSNAME`, `TIME`/`USER` (a "valid" sentence in the same section - `DFT` and the like are conditioned on their own structured rows).
- *Help-specification level:* `HLPARA`.
- *More than one level - need level-aware handling or a decision, NOT a plain table entry:* `CHANGE`, `CHGINPDFT`, `HLPTITLE` (not valid at the file level, *allowed* at the record level), `INDTXT`, `REFFLD`, `SFLMSGKEY`, `SFLPGMQ`, `SFLRCDNBR`, `SFLROLVAL`, `SFLSCROLL`, `TEXT`, `VALNUM`, `VLDCMDKEY`, `WRDWRAP`; `CHECK` (conditional per code - I-30). The table is keyed by name only; a level-scoped entry would need `keywordEditorHtml` to know its level.
- *In I-95's list but not confirmed by this scan (read directly):* `ALTNAME`, `CHOICE`, `GRDCLR`, `HOME`, `WDWTITLE`. (`GRDCLR` and `HOME` say "valid" in the sections found, so probably not.) *Confirmed here but not in I-95's list:* `CHRID`, `CSRLOC`, `GRDRCD`, `HLPSEQ`, `HTML`, `MNUBAR`, `SFLPGMQ`, `UNLOCK`, `WINDOW`.

Suggested order: the field-level-only batch next (37 names; several have a "valid" sentence or are conditioned by their own structured rows, so the structured field panels have to be checked as I-98 was), then `CSRLOC`/`SFLMODE`, then the multi-level ones last, since they need a design decision.
