...
| **I-48** | Investigation (split off from I-44's original finding): same re-read, for `MNUBAR`'s own DDS Reference section. `MNUBAR` appears on `PULLDOWN`'s own forbidden list (I-13) and has its own dedicated `MNUBARDSP`/`MNUBARSW`/`MNUCNL` sub-keyword handling (I-17/I-18/I-19/L76), but hasn't itself been checked for a blanket restriction on what else can coexist on a record that carries it. Independent of I-44 through I-47 - can run in parallel. **Claimed.** | I-44 (same original finding, split out) | in progress |
