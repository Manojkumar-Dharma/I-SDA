...
| **L5d** | **L5 piece: the record-level pickers** onto L1's repeatable-conditioned-instance component — the broadest of the four remaining pieces (spans multiple record-level panels, not one keyword family), so likely worth a further split into L5d-i/L5d-ii/etc. by whoever picks it up, the same way L5 itself was split into L5a-d here. **Before assuming L1's component is the right fit, check each keyword's own real SDA screenshot first** (per L5a/L5b/L5c's finding above) — several of these may turn out to be single-Resp-indicator flags that just need `flagRowHtml` instead, the same scope correction L5a/b/c needed. | L1 (done) | not started |
...
- L1 (the foundation component), L1a, L1b, L1c, L1d, L2, L3, and L4
  are all done. L5 itself (the umbrella row) is now split into four
  independently-workable pieces — **L5a (Input keywords), L5b
  (General keywords), L5c (Database reference), and L5d (the
  record-level pickers)** — plus **L6** (research + maybe implement
  the `WINDOW` picker's "Message line" row). **L5a, L5b, and L5c are
  now done** — all three turned out NOT to need L1's multi-instance
  component at all (their own real SDA screens show a single
  Resp-indicator slot per keyword, not a repeatable list); they
  needed the simpler per-keyword Conditioning toggle (`flagRowHtml`/
  `wireFlagRow`) instead, the same fix the parallel "Surface the
  per-keyword Conditioning toggle" effort had just landed everywhere
  else. **Only L5d (record-level pickers) remains open** from the L5
  family — check each keyword's own real SDA screenshot before
  assuming it needs L1's component rather than `flagRowHtml`, the
  same lesson L5a/b/c taught. **M1 through M5** (Menu designer) are
  five more independently-workable tasks, none dependent on each
  other or on any `L` task. That's up to six tasks (L5d, L6, M1-M5)
  that can all run in parallel right now with zero collision risk
  between them — whoever picks one up should mark it `in progress`
  here first. If a keyword turns out to be shared between two
  DSPF-designer panels the way `CHECK` was, see L1d's own row above
  for the pattern that keeps both panels safe (one shared
