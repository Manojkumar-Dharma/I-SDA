...
(function (root, factory) {
   ...
   *  RECORD_TYPES above). SFLMSG (message subfile) writes the identical
   *  SFL/SFLCTL keyword pair as plain SFL - a message subfile's DDS
   *  distinctiveness is per-FIELD (`SFLMSGID`/`SFLMSGKEY` on individual
   *  fields within the SFL record, added afterward through the normal
   *  field editor), not a distinct record-level keyword, so there's
   *  nothing different to write here at record-creation time. */
