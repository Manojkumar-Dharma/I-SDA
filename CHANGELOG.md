...
- **0.10.109** — I-30: opened and closed the field-level audit series' first task (character fields, the base 8-category set). Found and fixed a wide set of conditioning-eligibility bugs against IBM's own DDS Reference: `CHECK` was offering conditioning for every code when only `CHECK(ER)`/`CHECK(ME)` are valid (Keying Options + Validity Check, shared instance model); `RANGE`/`COMP`/`VALUES` were conditionable when IBM says none of the three ever are; `DSPATR` was conditionable even when its attributes were OID/SP-only (Colors & Attributes - a documented, deliberate edge case remains where a shared color+OID/SP-only state stays conditionable, since COLOR's own always-valid conditioning shares one condition set with DSPATR in this UI); `BLANKS`/`CHANGE` were conditionable when IBM says neither is (Input Keywords - `DUP` correctly is); and ten of the fourteen General Keywords rows (`ALIAS`/`INDTXT`/`DFT`/`CNTFLD`/`TEXT`/`FLDCSRPRG`/`HLPID`/`CHRID`/`IGCALTTYP`/`NOCCSID`) were conditionable when none of them are (only `DFTVAL`/`PUTRETAIN`/`OVRDTA`/`OVRATR` actually are); plus `DLTCHK`/`DLTEDT` (Database Reference), also never conditionable. Confirmed clean: `COLOR`, `CHGINPDFT`, `DUP`, `ERRMSG`/`ERRMSGID`, `REFFLD`; confirmed `KEYBRD` isn't a real DDS keyword at all (it's the field's own data-type column, position 35). Also fixed three index-only bugs in `build_index.py` (DSPATR's value list had two values that don't exist; `CNTFLD`'s parameter was mis-described as a field name instead of a numeric width; `DFTVAL` was marked repeatable when the code deliberately implements it as single-instance) and added `TEXT`/`HLPID` to the Character category, missing until now. New test file `i30CharacterFieldConditioningAudit.test.js` (55 checks, registered in `package.json`), verified to fail 34 ways against pre-fix code via `git stash`. Several real, sourced gaps logged but deliberately not fixed this pass (CHKMSGID's missing validity-check dependency guard, CHRID's/IGCALTTYP's unenforced mutual-exclusion lists, DUP's missing floating-point restriction, and MSGID's unusual position-dependent mandatory/forbidden conditioning rule) - flagged for a follow-up rather than rushed. Full suite: 4004/4004 (up from 3949).
- **0.10.110** — I-33: Constant fields, including the system-value sub-form. `PAGNBR` was never a real DDS keyword (same bug class as I-2's `PRTFILE` — appears nowhere in the DDS Reference, likely inherited from RPG's unrelated `*PAGNBR` special word) — removed with no replacement. `MSGCON` (Message Constant), a genuinely documented 6th way to supply a constant's value alongside `DFT`/`DATE`/`TIME`/`SYSNAME`/`USER`, was entirely missing — added as a full peer of the existing system-value sub-form, with its own structured Length/Message ID/Message file/Library UI, new `DspfWriter.parseMsgConParams`/`formatMsgConParams` helpers, and its own design-preview placeholder. Also fixed the shared `GENERAL_FIELD_KEYWORD_ROWS` list gating wrong in both directions for every field kind: `DFTVAL`/`CNTFLD`/`FLDCSRPRG`/`CHRID`/`IGCALTTYP` (each individually documented as named-field-only) were being offered on constants, while `HLPID` (documented as constant-only) was being offered on named fields too. Rebased onto I-30's own same-array change (`GENERAL_FIELD_KEYWORD_ROWS`' new conditionable-eligibility column) - merged into one 6-element row shape carrying both dimensions rather than picking one.
- **0.10.108** — I-16: regenerated `KEYWORD-INDEX.json`/`KEYWORD-INDEX.md`/`KEYWORD-LOOKUP.json` against the codebase as it stood after I-7 through I-29, closing out the record-level audit series. Corrected several stale entries left behind by earlier fixes: dropped the standalone `PRTFILE` keyword (I-2 - it was never real DDS syntax, only ever `PRINT`'s own parameter) at both file- and record-level; added file-level `HLPRCD`/`MOUBTN`/`VALNUM`/`WRDWRAP` (I-5); marked record-level `MNUBARDSP` (I-17) and record-level `HLPTITLE` (I-27) as repeatable, independently-conditioned instance lists; removed `KEEP` from the `SFL`/`SFLCTL` subfile panels' own keyword lists now that it's a hint pointing at its sole surviving live copy on the base Record Keywords General tab (I-25); added `SFLSNGCHC`/`SFLMLTCHC`/`SFLSCROLL` (I-26); and removed the separate "Menu-Bar record - Menu-Bar Display Keywords" category and `MNUBARDSP`'s duplicate row on the `MNUBAR` record's own General tab, confirmed a documentation artifact rather than a second live UI surface (I-14, closed here). No runtime code touched — `build_index.py`/`build_lookup_and_md.py` and their three output files only; full suite unaffected (3949/3949).
- **0.10.107** — I-26: added `SFLSNGCHC`/`SFLMLTCHC` (Subfile Single/Multiple Choice Selection List, record-level `SFLCTL` keywords — mutually exclusive with each other and with `SFLDROP`/`SFLFOLD`) and `SFLSCROLL` (field-level, alongside `SFLRCDNBR`/`SFLROLVAL` — mutually exclusive with both on the same field, unique across the whole record) — I-15's own audit had found these entirely missing. Both keywords' own `*RSTCSR`/`*NORSTCSR` default (and `SFLSNGCHC`'s own `*AUTOSLT`/`*NOAUTOSLT`/`*AUTOSLTENH` default) flips when the `SFLCTL` record is defined in a pulldown — surfaced as live default-hint text rather than silently guessing, with the explicit non-default value still writable either way. Corrected a mix-up in `keywordFixes.md`'s own I-9 write-up along the way: the `AUTOSLT` group belongs to `SFLSNGCHC` only, not `SFLMLTCHC`.
- **0.10.106** — I-28: base Record Keywords panel's `KEEP` row no longer offers an invalid Conditioning toggle (I-9 had already fixed this on the SFL/SFLCTL copies but not the base copy, now the sole surviving copy after I-25's de-dup) — new `DspfWriter.keepMutexConflictReason` also hard-blocks (alert + revert) `KEEP` from being combined with `ALWROL`/`CLRL`/`SLNO` on the same record, in either direction, per the DDS Reference.
- **0.10.105** — I-27: record-level `HLPTITLE` rebuilt as a genuine repeatable, independently-conditioned instance list — reuses the same generic `DspfWriter.getRepeatableKeywordInstances`/`setRepeatableKeywordInstances` primitive I-17 built for `MNUBARDSP` (HLPTITLE needed no bespoke get/set pair, being a single plain quoted-text parameter). Replaces I-21's own single-instance row, which correctly conditioned ONE record-level HLPTITLE but couldn't represent a second at all; up to 15 per record are now supported, matching IBM's own documented shape (all must be optioned once there's more than one — surfaced as a non-blocking hint, not hard-enforced, same posture MNUBARDSP's own repeatable panel takes for its analogous rule).
- **0.10.104** — I-29: researched the "Roll" column on real SDA's own "Define Display Layout" screen (`SFLSIZ`/`SFLPAG`/`SFLLIN`) — found decisive documentation in IBM's own Screen Design Aid manual (SC09-2604-00): it's SDA's own generic navigation widget for scrolling through any "duplicate keyword" (a keyword allowed to be specified more than once on a record), not a hidden DDS parameter. `SFLSIZ`/`SFLPAG`/`SFLLIN` qualify because each can repeat once per `DSPSIZ` display-size condition (I-22's own finding) - I-22's per-size input rows are the natural GUI equivalent of the same feature. No code change needed.
- **0.10.103** — I-23: verified the ~9 (actually 11, `SFLNXTCHG`/`TEXT` were miscounted out) keywords only *implied* to conflict with `SFLMSGRCD` — none carry an individual "cannot specify" statement, so no new hard-block guards. `LOGINP`/`LOGOUT` ARE individually documented as merely *ignored* (not a compile error) on a message subfile — new advisory-only `DspfWriter.loginpLogoutSflMsgRcdIgnoredNote`, surfaced on the SFLMSG panel. `KEEP` (own exclusion list omits `SFLMSGRCD`) and `TEXT` (own section explicitly allows it at record level except two special fields) were individually ruled out; `CHANGE`/`CHECK(AB)`/`CHECK(RL)`/`CHGINPDFT`/`INDTXT`/`SETOF`/`SETOFF` have no individual statement either way and were deliberately left untouched rather than guessed at.
- **0.10.102** — I-22: `SFLSIZ`/`SFLPAG`/`SFLLIN` display-size (`*DSx`) conditioning — `getSflDisplayLayout`/`setSflDisplayLayout` rebuilt onto the same `{primary, bySizeName}` shape `SFLMSGRCD`/`MSGLOC` already use, fixing the same silent-data-loss bug L79 found for `SFLMSGRCD` (only the first matching instance was ever read). New per-size input rows on the SFLCTL Display Layout panel, shown only when the file declares 2+ `DSPSIZ` sizes. `SFLSIZ`'s own field-name/display-size mutual exclusion ("cannot use display size condition names... when a program-to-system field is used") is now hard-blocked (alert+revert). Also corrected `SFLPAG`'s own placeholder text, which incorrectly claimed a field-name form that its DDS Reference section never documents.
- **0.10.101** — I-20: repeatable Indicator-instance model made kind-aware — the shared `repeatableConditionedInstancesHtml`/`wireRepeatableConditionedInstances` primitives gained an optional per-instance `isConditionable` predicate (default: always true, so no other caller is affected); the record Indicator-keywords panel now uses it to hide the Conditioning toggle for `VLDCMDKEY`/`SETOF`/`CHANGE`/`INDTXT` (IBM: option indicators not valid for any of the four), while existing pre-existing conditioning on those keywords is left untouched. Also closes I-13's own deferred finding that `CLEAR` — living in this same shared component — was never guarded against `PULLDOWN`'s forbidden-keyword list: a kind-switch/edit to `CLEAR` on a PULLDOWN record is now blocked (alert+revert), and "+ Add indicator keyword" falls back to a `HOME` default there instead of the usual `CLEAR` so the Add button never silently no-ops.
- **0.10.100** — I-25: `KEEP` duplicated across 4 record-type panels (base General tab, SFL, SFLMSG, SFLCTL) — consolidated to a single live control on the base tab, with a hint on each of the other three pointing back to it. Went further than Task R3's own `CHGINPDFT` precedent, which left the SFLMSG panel's own copy live; this removed it there too. Discovered along the way: the base tab's own `KEEP` row still offers an invalid Conditioning toggle (I-9 fixed this on the SFL/SFLCTL copies but never the base one, now the sole survivor) — split to new task I-28, not fixed here.
- **0.10.99** — I-24: `WINDOW` cannot be specified for the record named by file-level `PASSRCD` — new `DspfWriter.passrcdWindowConflictReason` (pure name-vs-name check) wired at both reachable on-transitions: the "+ Add record" wizard blocks creating a new `WINDOW`-type record whose name matches the current `PASSRCD` value, and editing file-level `PASSRCD` is blocked (alert + revert) from naming a record that already carries `WINDOW`. Confirmed the DDS Reference states the identical restriction for `ALWROL`/`CLRL`/`SLNO` too — split to a follow-up finding, not fixed here.
- **0.10.98** — I-21: `CSRLOC`/record-level `HLPTITLE` missing conditioning — `getFileTwoFieldKeyword`/`getFileQuotedText`'s shared setters didn't carry a `conditions` parameter at all (and unconditionally reset it to `[]` on every text edit, the same data-loss class `setFileFlagKeyword` had before its own fix); both now support the same preserve-when-omitted `conditions` contract, and CSRLOC/record-level HLPTITLE gained a Conditioning toggle (file-level HLPTITLE and HLPSEQ correctly stay un-conditioned, per IBM). Record-level HLPTITLE's own richer repeatable-conditioned-instance shape (up to 15 per record) is bigger, separate scope — split to new task I-27.
- **0.10.97** — I-19: `MNUBAR` field-shape structural constraint — new `DspfWriter.mnubarFieldShapeNote` advisory note (surfaced on the MNUBAR tab, not a hard block, per L83's cross-object precedent) flags a menu-bar record that's missing its one MNUBARCHC-bearing field, has more than one, or carries an extra displayable field beyond it. Confirmed iSDA's field-adding UI had no such structural awareness before this fix.
- **0.10.96** — I-18: `MNUBARSW`/`MNUCNL` mutual CA-key exclusion guard — both keywords' own DDS Reference sections forbid sharing a CAnn key with each other, and state the file-level form extends to every record, so the new `DspfWriter.mnuBarKeyConflictReason` checks all four scope combinations (file-vs-file, file-extends-to-record both directions, same-record) rather than just a same-record check. Blank CA-key boxes correctly resolve to each keyword's own documented default (CA10/CA12) before comparing.
- **0.10.95** — I-17: `MNUBARDSP` repeatable-conditioned-instance support — replaced the single-instance row with a repeatable, independently-conditioned instance list (reusing the same generic primitive MOUBTN already uses), covering IBM's own "more than one MNUBARDSP keyword can be specified on the record if all are optioned." Both parameter shapes (3-name on a plain record, single pull-down-field name on a MNUBAR record) preserved.
- **0.10.94** — I-15: combination record-type recheck (`WNDSFCTL`/`PDNSFLCTL`/etc.) — confirmed `WINDOW`+`SFLCTL` and `PULLDOWN`+`SFLCTL` are both correctly built (matching IBM's explicit allow/forbid text) with zero cross-contamination, no code change needed. Found `SFLSNGCHC`/`SFLMLTCHC` each have a pull-down-conditional parameter default IBM documents, but neither keyword exists in iSDA yet — split to new task I-26 rather than fixed here.
- **0.10.93** — I-12: `WINDOW` record-level keyword audit — `ALWROL`/`ASSUME` now hard-blocked from turning on on a record that already carries `WINDOW` (IBM: WINDOW is not allowed on a record format with either of these); `WDWBORDER` file-level/record-level sharing, `WINDOW`'s own parameter shapes, and `RMVWDW`/`USRRSTDSP`'s conditioning all confirmed already correct. `PASSRCD` cross-reference restriction flagged, not fixed.
- **0.10.92** — I-13: `PULLDOWN` record-level keyword audit — IBM's own DDS Reference lists 27 keywords incompatible with a PULLDOWN record; hard-blocked the 22 that live on the shared RECORD panel (`INZRCD`/`ALTNAME`/`ALWROL`/`ASSUME`/`HLPCLR`/`HLPSEQ`/`ALARM`/`INVITE`/`ALWGPH`/`FRCDTA`/`SLNO`/`CLRL`/`RTNDTA`/`OVERLAY`/`PUTRETAIN`/`PUTOVR`/`OVRDTA`/`OVRATR`/`MDTOFF`/`ERASEINP`/`ERASE`, plus PULLDOWN's own "on" checkbox), bidirectionally (either direction of the on-transition is blocked). `CLEAR` flagged (repeatable Indicator-instance model isn't kind-aware); `MNUBAR`/`SFL`/`USRDFN`/`WINDOW`/`WDWTITLE` covered from PULLDOWN's own side only.
- **0.10.91** — I-14: `MNUBAR` (menu bar record) keyword audit — Conditioning toggle removed from `MNUBAR` itself (IBM: not eligible), whose placeholder now shows the confirmed `*SEPARATOR`/`*NOSEPARATOR` parameter syntax; `MNUBARSW`/`MNUCNL`/`MNUBARDSP` confirmed already correct. `KEYWORD-INDEX.json`'s `MNUBARDSP` duplicate-listing question resolved as a documentation artifact only (left for I-16). Repeatability, CA-key mutual exclusion, and MNUBAR's field-shape constraint flagged as follow-ups, not fixed here.
