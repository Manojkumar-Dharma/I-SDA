...
- **0.10.145** — I-68 (raised by I-38): `HLPRTN`'s file-level checkbox now gets the reverse direction of I-38's `HLPDOC`/`HLPRTN` mutual exclusion ("You cannot specify HLPDOC with … HLPRTN") — ticking it while `HLPDOC` is present is blocked with an alert. Checked only on the off→on transition, so a hand-edited file already carrying both stays editable and turning `HLPRTN` off is never blocked; `HLPRCD`/`HLPPNLGRP` alongside `HLPRTN` stay allowed (IBM ranks those by priority, not prohibition). `commitIndicatorTextRow` gained an optional `conflictFn`; only `HLPRTN` uses it. Cross-level scope (record-level `HLPRTN`, H-spec-level `HLPDOC`) is unstated in IBM's text and logged as a deferred finding. New `i68HlprtnReverseGuard.test.js` (29 checks), confirmed to fail against pre-fix code.
- **0.10.144** — I-66 (raised by I-57): `PSHBTNCHC` choice-text validation, from the DDS Reference. New `analyzePshbtnchcText` follows IBM's `>` mnemonic / `>>` literal rules (its own examples come out as documented). Errors, enforced by both the row editor and the raw keyword editor: more than one mnemonic, a `>` with no mnemonic character, a blank or double-byte mnemonic; `&FIELD` texts are never checked, and only new text is checked so unrelated edits to an already-invalid row are not blocked. Warnings in the Push button field panel: two choices sharing a mnemonic (IBM defines the fallback, so it is not an error), and an estimated fit check against the smallest `DSPSIZ` width or window width using the designer's own layout (IBM gives no formula). New `i66PshbtnchcTextValidation.test.js`.
- **0.10.143** — I-64 (follow-up from I-57): `PSHBTNFLD`'s own ten-keyword whitelist was only enforced on the raw keyword editor, turning `PSHBTNFLD` on, and Choice selection type - every other structured field panel (Color & attributes, Keying options, Edit code/word, validity checks, Reference, the General keyword rows, etc.) could still add a non-whitelisted keyword to a push-button field. New `DspfWriter.pshbtnfldNewConflictReason`, a diff-based backstop with the exact same shape as I-58's own `wrdwrapNewConflictReason`, wired into `commitEdit`'s existing choke point - one function closes the gap for every panel at once rather than a per-panel sweep. Conflicts already present before an edit are not re-reported. New `i64PshbtnfldPanelWhitelist.test.js`.
- **0.10.142** — I-65 (raised by I-57): a push-button field (`PSHBTNFLD`) now has editors for `CHCCTL` and `CHCAVAIL`/`CHCUNAVAIL`, which its whitelist allows but which were previously reachable only through the raw keyword editor. A new CHCCTL-only editor (one row per `PSHBTNCHC` choice, with the reference's message-parameter rules and a flag for a `CHCCTL` with no matching choice) replaces the choice editor there, because that editor also writes `CHOICE`/`CHCACCEL`; the colours editor is reused restricted to Available/Unavailable, because `CHCSLT` is not allowed on `PSHBTNFLD`. New `i65PshbtnChoiceControlColors.test.js`.
- **0.10.141** — I-63 (raised by I-57): `SNGCHCFLD`/`MLTCHCFLD`'s `*NUMCOL`/`*NUMROW`/`*GUTTER` were written as `*NUMCOL(3)` (invalid DDS) and could not be read back in IBM's own `(*NUMCOL 3)` form, so Apply silently dropped them. Reader/writer now use IBM's shape via helpers shared with `PSHBTNFLD`'s reader (the old shape is still read and corrected on the next Apply); the writer takes `*NUMCOL` or `*NUMROW`, never both, and `*GUTTER` only alongside one; the Choice selection type Apply blocks the three invalid combinations. New `i63ChoiceLayoutParamsShape.test.js`.
