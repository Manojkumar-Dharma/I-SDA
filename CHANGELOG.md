...
## 2026-09-10 — Feature (L76): MNUBARDSP got structured inputs instead of one flat free-text parameters box

- **0.10.64** — The record-level General tab's "Menu-Bar display (MNUBARDSP)" row used to be a single free-text box the person had to hand-type 1-3 space-separated names into, in the right order, with no validation. Real DDS actually gives MNUBARDSP two different parameter shapes depending on whether the record itself carries `MNUBAR`: on a MNUBAR record it's just one optional "Pull-down input field" name (unchanged, still the existing checkbox+single-box shape, just relabeled); on any other record it's 3 Name inputs - Menu-bar record, Choice field, Pull-down input field (optional) - via new `getMnubardspFields`/`setMnubardspFields` in `dspfWriter.js`, a 3-field sibling of the existing `getFileTwoFieldKeyword`/`setFileTwoFieldKeyword` pair. The row's existing Conditioning toggle (indicator conditioning) was already correct and untouched. New `.three-col` CSS class added alongside `.two-col` for the 3-input layout. Full suite: 43/43 files, zero failures.
...
## 2026-09-09 — Bug fixes (L69-L70) + feature (L71): toolbar alignment, more filename/label removal, clickable MNUCMD source

- **0.10.61** — **L69**: fixed the toolbar's Save button and IBM i badge sitting visibly misaligned - `.codefori-badge`'s own `margin-bottom: 10px` (correct for its original vertical-stacking use in the aside) was throwing off `align-items: center`'s positioning in the toolbar's flex row; scoped override zeroes it out there only. **L70**: removed the redundant file name label above the IBM i badge in the DSPF designer's classic-UI left panel and the Menu Designer's own left panel (on top of L62's earlier toolbar-only removal), plus the now-purposeless "File" labels each panel had directly above/near that badge (the DSPF designer's had a real click-to-open-file-properties shortcut, but the properties panel's own `crumb-file` breadcrumb already does the exact same thing, so nothing was lost; the Menu Designer's was purely decorative). **L71**: the MNUCMD command source name above the Menu Designer's "Compile Menu" button is now clickable and opens that companion source - using the exact source library/file `getMenuCommandMemberUri()` already correctly derived from the open MNUDDS document. Full suite: 43/43 files, zero failures.
...
## 2026-09-06 — Bug fixes (L61-L65): Find field dropdown width, toolbar filename clutter, Save/Compile errors in canvas, missing accordions, M10/M11 Immed visibility

- **0.10.57** — Five bug fixes reported directly against New UI (one item, L64, also applies to classic UI since it's shared HTML/JS). **L61**: the pinned toolbar's "Find field" results dropdown rendered edge-to-edge across the whole canvas instead of under the search box — `#propsFindFieldRow` (P5e) sits outside `.panel-body` but never got its own `position: relative`, so its dropdown's `position: absolute; left:0; right:0` fell back to the viewport as its containing block; fixed with one CSS rule. **L62**: removed `#toolbarFileStatus` (a redundant second copy of the filename sitting directly in front of the toolbar's IBM i connection badge) outright — confirmed via grep that nothing else ever read or updated it. **L63**: Compile/Save failures now also render inline in the canvas below the "Click a field..." hint (new `#canvasMessage`, red-styled), not only as a VS Code toast easy to miss — `compileDspf()`/`handleSaveDocument()` (`extension.ts`) take an optional `webview` and `postMessage` a `compileResult`/`saveResult` on failure in ADDITION to the existing `showErrorMessage` calls, so command-palette invocation with no designer open still works exactly as before. Success is deliberately NOT mirrored into the canvas (the toast alone is enough there); `render()` clears any stale message on the next re-render. **L64**: Color & attributes, Validity check, Check message identifier, and Edit code/word/mask were the only field-props panels still always-expanded, unlike Error messages/Keying options right next to them — now the same collapsible `<details class="props-accordion">` treatment, `validityAndEditHtml` split into three independent accordions instead of one blob covering all three; no element-id changes, so all existing panel tests kept working unmodified. **L65**: CHECK's M10/M11 "Immed" checkbox no longer shows until M10/M11 itself is checked (previously always visible regardless) — `dspfWebview.test.js`'s M10/Immed scenario updated to check M10 first, confirm Immed then appears, before toggling it. Full suite re-verified: 41/41 files, zero failures.

## 2026-09-05 — Task L60: "Find keyword" now finds keywords to ADD, not just ones already set

- **0.10.56** — Fixed a real usability gap in the "Find keyword" quick-nav (Task L37): it searched rendered `label`/`.section-label`/`.keyword-chip` text, which works for plain flag rows but leaves several panels unsearchable by their own DDS keyword code — Color & Attributes never shows the literal text "COLOR" or "DSPATR" (just "Color & attributes" and VALUE-code checkboxes like HI/RI), Validity Check's RANGE/COMP/VALUES hint text lives in a `<div class="hint-small">` the old selector never scanned, and the record Indicator panel's CLEAR/PAGEDOWN/PAGEUP/HOME/HELP/HLPRTN/VLDCMDKEY/SETOF/CHANGE/INDTXT codes were only visible as `<option>` text inside a closed `<select>`. For all of these, the keyword was only ever findable via the Advanced/raw keywords editor's own `.keyword-chip` — which exists only once that keyword is already set on the field/record. That's backwards for a keyword *finder*, whose whole point is jumping to where you'd add one. New `webviewClientHelpers.js` helper `dataKwWrap(codes, html)` tags each affected panel's own outer container with `data-kw="CODE1 CODE2 ..."`, independent of whatever text is actually visible; `findKeywordMatches` (buildWebviewTemplate.js) now also matches against that attribute. Fixed panels: Colors & Attributes, Validity Check, CHECK-based keying/validity panels, Window Border, and the record Indicator panel. Checked Choice colors & attributes (CHCAVAIL/CHCUNAVAIL/CHCSLT) too — already correctly labeled, no change needed. New regression tests confirm DSPATR/COLOR/RANGE (field-level, on a field with none of those set) and VLDCMDKEY (record-level, on a record with none set) are now all found by the finder. Full suite re-verified, zero failures.

## 2026-09-05 — Docs: Keyword Index (docs/sda-reference/keyword-index/)

- **Docs-only, no version bump** — Added a structured keyword inventory: `KEYWORD-INDEX.json` (200 keyword entries, 159 unique keyword names, 47 categories, organized by level → category exactly matching iSDA's own UI tabs), `KEYWORD-LOOKUP.json` (flat keyword-name → location map for quick lookup), and `KEYWORD-INDEX.md` (human-readable rendering of both), plus the two Python scripts that generate them and a README. Built by reading `webviewClientHelpers.js`'s panel-building functions directly (`fileKeywordsPanelsHtml`, `recordKeywordsPanelsHtml`, `sflCtlPanelsHtml`, `windowPanelsHtml`, `keyingOptionsHtml`, etc.) — the same code driving iSDA's own UI — rather than reconstructed from the reference screenshots or memory. Intended for two uses: side-by-side comparison against real IBM i SDA's own screens, and as the data layer for a future in-app "quick find keyword" navigation feature (not yet built). Documentation only — no extension code changed, nothing to compile or test.

## 2026-09-05 — A-series audit: ALTNAME/RMVWDW/USRRSTDSP/KEYBRD gaps fixed (L57-L59)

- **0.10.55** — Completed the A1/A2 SDA screenshot keyword-inventory audit L22 started but left ~130 record-level and ~25 field-level screenshots unreviewed. A1 (record-level variants: SFL/SFLCTL/SFLMSG, WINDOW+borders, PULLDOWN, MNUBAR, WNDSFL/WNDSFCTL/PULDWNSFL/PDNSFLCTL, USRDFN) found `ALTNAME` (record-level quoted-string keyword, shown on every "Select Record Keywords" screen) and `RMVWDW`/`USRRSTDSP` (record-level flags, shown on WNDSFCTL's general screen) completely missing from the codebase. A2 (remaining field-level screens) found KEYBRD used one unconditional 11-value list for every field, when real SDA shows a different, shorter S/N/Y/I/D list for numeric fields specifically. **L57**: ALTNAME added next to the record TEXT input (`getFileQuotedText`/`setFileQuotedText`, same pattern as TEXT/HLPTITLE/WDWTITLE). **L58**: RMVWDW/USRRSTDSP added to the Window Parameters panel (`getFileFlagKeyword`/`setFileFlagKeyword`, no new dspfWriter.js functions needed). **L59**: `keyingOptionsHtml` now takes the field's `dataType` and branches between the numeric and character KEYBRD lists (missing/unrecognized dataType falls back to the wider character list). Existing QTY (numeric) regression test corrected to expect the right list; new DESCR (character) regression test added to cover the other branch. Everything else in both audit scopes checked out — several apparent gaps turned out to be pre-existing, deliberately-documented deferrals (multi-instance SFLSIZ/SFLPAG/SFLLIN conditioning, SFLMSGID's trailing columns, REFFLD's separate resolver feature, the Menu-bar choice variable-argument forms). Full suite re-verified, zero failures.
...
## 2026-09-05 — Task P5i: aside panel hidden under modern UI, two-column layout (closes the P series)

- **0.10.52** — Task P5i (LIMITATIONS-PLAN.md's P series), the final row in the aside-migration group: `<aside>` is now hidden entirely under modern UI (`body[data-ui-style="modern"] aside { display: none; }`), NOT removed from the DOM. P5's own inventory of aside content never listed `#placeFieldBtn`/`#placeConstantBtn`/`#newRecordToggleBtn`+`#newRecordForm`/`#addFromDbBtn`/`#placementHint` - those are covered by P1-P4's toolboxFab instead, whose items proxy-click their real aside counterpart rather than duplicating its logic. Deleting the DOM outright would have silently broken every one of those proxies; `element.click()` still fires on a `display:none` element, so aside becomes inert-but-functional plumbing under modern instead, same "shared markup, hidden via CSS" pattern `#toolboxFab` already uses in reverse. `applyPanelCollapse()` now branches on UI style - two-column grid under modern, the original three-column formula under classic, unaffected - and both style-toggle buttons recompute it live on switch, no reload needed. New test file `src/test/toolboxAsideHidden.test.js`. This closes out the entire P series (P1-P5i) - New UI's aside migration is complete.

## 2026-09-05 — Task P5g: new "View" accordion in the persistent accordion zone

- **0.10.51** — Task P5g (LIMITATIONS-PLAN.md's P series): `#viewAccordion` added to P5a's `#propsAccordionZone`, as a sibling of P5h's UI Settings accordion (landed concurrently, merged in cleanly). Genuine duplicates of the compare-mode/overlay/SFLPAG-preview toggles, ruler/crosshair toggles, compare-record list, and conditioning-indicators list - not a relocation, since classic UI never shows the accordion zone at all. Checkboxes forward to their real counterpart; a new `syncViewToggles()` mirrors state back (checked + hidden-class visibility), called from `render()` and directly from ruler/crosshair's own listeners (neither calls `render()`). `rebuildIndicatorListFromSet`/`renderCompareRecordList` now build into both containers from the same underlying data. Caught and fixed one real gap: `compareModeToggle`'s listener toggled the aside `compareRecordList`'s hidden class but never the toolbar duplicate's. `propsPanelShell.test.js` updated again for the now-populated accordion zone. New test file `src/test/toolbarViewAccordion.test.js`. Full suite: 41/41 files, zero failures.

## 2026-09-05 — Task P5h: UI Settings accordion (style/theme) migrated into the persistent accordion zone

- **0.10.49** — Task P5h (LIMITATIONS-PLAN.md's P series): a second `#toolbarUiSettingsAccordion` added inside P5a's `#propsAccordionZone`, carrying its own `#toolbarUiStyleToggle`/`#toolbarUiThemeRow`/`#toolbarUiThemeSelect`. This row's own text says the accordion "relocates as-is" (a move), but - like P5b/c/d/f before it - it's actually a genuine second element mirroring the aside original 1:1, kept in sync, coexisting until P5i finally deletes the aside outright: a literal move now would be the one migration that couldn't be undone, and would leave classic UI (which keeps the aside forever) with no working style toggle at all in the meantime. Unlike P5b/c/d, the two style-toggle buttons and two theme selects here also sync with EACH OTHER directly (not just with the extension host) - clicking or changing either the toolbar or the aside control updates both immediately, with only one `setUiStyle`/`setUiTheme` message posted per interaction. `propsPanelShell.test.js` updated to match, since `#propsAccordionZone` is no longer empty by default. New test file `src/test/toolbarUiSettings.test.js`.

## 2026-09-05 — Task P5f: aside branding folded into a live "Screen Design · N records" toolbar title

- **0.10.48** — Task P5f (LIMITATIONS-PLAN.md's P series): a new `#toolbarTitle` line added to `#propsPinnedToolbar`, `flex-basis: 100%` so it sits on its own row ahead of Save/Compile/etc. Unlike P5b/c/d's own migrations (a genuine second element mirroring an aside original 1:1), this is a redesign rather than a duplicate: the aside's `h1` ("IBM i · DDS") is dropped as purely decorative, `h2`'s "Screen Design" survives, now paired with a live record count neither ever showed - "Screen Design · N records" (singular at exactly one). The count genuinely changes as records are added/removed (unlike `#toolbarFileStatus`'s filename, baked in once and never touched again), so a new `updateToolbarTitle()` runs on every `render()` alongside `rebuildRecordSelect()`. The aside's own `h1`/`h2` are untouched - same coexist-until-P5i rule every P5b-h migration has followed. New test file `src/test/toolbarTitle.test.js`.
## 2026-09-05 — Task P5e: Find-field search (+ its results dropdown) duplicated into its own row under the pinned toolbar

- **0.10.50** — Task P5e: `#propsFindFieldRow` added as a new sibling of `#propsPinnedToolbar` (P5a), both outside `.panel-body`, so it survives a full panel collapse the same way Save/Compile/Record select already do - P5a's own doc comment had already earmarked this control for that camp. Block-level rather than the toolbar's own flex row, since the results dropdown needs to sit below the input. Unlike P5d's Record/Screen-size selects (referenced from around a dozen other call sites, which is why that task kept one authoritative element and duplicated only the DOM), nothing outside the Find-field widget's own wiring reads its elements by name, so the whole wiring block was factored into a reusable `wireFieldSearch(inputEl, resultsEl)` function and called twice - once for the untouched aside pair, once for the new `#toolbarFieldSearchInput`/`#toolbarFieldSearchResults` pair - giving each instance its own independent matches/active-index/open state rather than two inputs sharing one array. New test file `src/test/toolbarFindFieldSearch.test.js`, covering the toolbar twin's full behavioral parity, genuine independence from the aside instance, the new row's placement/collapse-survival/classic-UI gating, and survival of a `#propsBody` selection-triggered swap.

## 2026-09-05 — Task P5d: Record select + Screen-size select duplicated into the pinned toolbar

- **0.10.47** — Task P5d: `#toolbarRecordSelect`/`#toolbarSizeSelect`/`#toolbarSizeSelectRow` added to the pinned toolbar (P5a), genuine duplicates of the aside originals rather than a relocation. The first version of this task tried relocating the aside originals straight into the toolbar instead - which passed every existing test, but would have made switching records completely unusable under classic UI, since the toolbar is invisible unless the New UI style is active. Caught on review before shipping. Reverted to duplication, but without touching the ~14 existing call sites that read/write `recordSelect.value`: the aside originals stay the one authoritative element, and the already-per-render `rebuildRecordSelect()`/`rebuildSizeSelect()` wrapper functions are the single place both copies get rebuilt/synced from; selecting from a toolbar duplicate forwards to the real element's own `change` listener rather than re-implementing it. New test file `src/test/toolbarRecordSizeSelect.test.js`, leading with the classic-UI regression scenario this almost shipped with.

## 2026-09-05 — Task P5c: file status label + Code-for-IBM-i badge migrated into the pinned toolbar

- **0.10.46** — Task P5c (LIMITATIONS-PLAN.md's P series): `#toolbarFileStatus`/`#toolbarCodeForIBadge` added to P5a's `#propsPinnedToolbar`, ahead of P5b's Save/Compile buttons. The file status label needs no message-driven sync at all - the aside original is itself static, set once from the filename token at render time and never touched afterward - so the toolbar copy is just a second static element with the same content. The connection badge does need syncing: `updateCodeForIBadge` now applies the same computed class/text/title to both the aside badge and its toolbar counterpart from one `codeForIStatus` message, same "compute once, apply to both" pattern P5b's own Save/Compile sync already established. Genuine second elements, not proxies, for the same reason P5b's own entry gives. New test file `src/test/toolbarFileStatusBadge.test.js`.

## 2026-09-05 — Task P5b: Save + Compile Display File buttons migrated into the pinned toolbar

- **0.10.45** — Task P5b (LIMITATIONS-PLAN.md's P series): `#toolbarSaveBtn`/`#toolbarCompileBtn` added to P5a's `#propsPinnedToolbar`, giving both actions a home that survives the right panel's own "Hide panel" collapse (P5's own risk (2)) as well as `#propsBody`'s wholesale empty-state swap. Unlike P1's toolboxFab items, these are genuine second buttons with their own click handlers rather than proxies that `.click()` the aside originals — Save/Compile's own handlers are one-line `postMessage` calls, not complex logic worth avoiding duplicating, and P5i eventually deletes the aside originals outright, which a proxy would need re-wiring for anyway. Shared state (the dirty-indicator text/class from `dirtyState` messages, the connection-gated `hidden` class from `codeForIStatus` messages) is computed once and applied to both elements from the same `updateSaveButtonDirtyState`/`updateCodeForIBadge` functions, so the two buttons can never drift out of sync. `propsPanelShell.test.js` (P5a) updated to match — `#propsPinnedToolbar` is no longer empty by default, so its "invisible while empty" checks now clear/repopulate it directly to prove the underlying `:not(:empty)` CSS mechanism still holds, rather than relying on default emptiness that no longer exists. New test file `src/test/toolbarSaveCompile.test.js`.

## 2026-09-05 — Task P5a: foundation shell for migrating the aside panel into the right-side props panel

- **0.10.44** — Task P5a (LIMITATIONS-PLAN.md's P series): two new empty regions inside `#propsPanel`, giving it a third structural zone beyond the existing `#propsBreadcrumb`/`#propsBody` pair, so P5b-h's migrated aside-panel content has somewhere to live that survives both of P5's own named risks. `#propsPinnedToolbar` sits as a sibling of `#rightPanelToggle`, placed BEFORE `.panel-body` - structurally exempt from `.panel-collapsed .panel-body { display: none }` with no JS change needed, since the exemption falls out of DOM placement alone (P5b/c/d/e/f - Save, Compile, status, Record select, screen-size select, the branding line - all land here). `#propsAccordionZone` sits between the existing mod-tracking-row and `#propsBreadcrumb`, still inside `.panel-body` on purpose - it only needs to survive `#propsBody`'s own wholesale empty-state swap, not a full panel collapse (P5g/h - the new View accordion and the relocated UI Settings accordion land here). Both default to `display: none`, shown only under `body[data-ui-style="modern"]` AND `:not(:empty)` - present in the DOM under classic (same pattern P1 already established for modern-only controls) but rendering as nothing while empty, so no blank gap appears before P5b-h actually populate them. Collapsed-rail width tuning is deliberately deferred to whichever sub-task first lands content that doesn't fit the existing 28px rail. New test file `src/test/propsPanelShell.test.js`. Also fixed in this pass: a CHANGELOG ordering bug found while adding this entry - the L54/L55 (0.10.43) entry had landed below several lower-numbered entries instead of at the top, breaking strict descending order; moved back into place.

## 2026-09-05 — Task L54/L55: comments could vanish when inserted at a chosen line, plus the "Line #" box's spinner arrows removed

- **0.10.43** — Two more direct reports, both confirmed by direct reproduction:
  - **Task L54**: a comment added without a line number showed up immediately, but one added WITH a target line number did not. Root cause: `DspfWriter.addComment`'s `desiredLine` path spliced the raw comment line in verbatim at whatever line number was typed, with no awareness that some physical lines are the middle of a DDS function-area (+/-) continuation - a keyword's parameter list wrapped onto a second physical line. Splicing a comment (column 7 = `*`) into the middle of one desyncs `DspfParser`'s own continuation resolution: the interrupted keyword's parameter list silently truncates at the dangling `+`/`-`, and its continuation gets re-parsed as an unrelated new keyword-only line instead - reproduced directly: a `VALUES` keyword's parameter list truncated mid-string and a garbage keyword spawned out of the leftover text. The comment (and whatever it landed next to) appearing to vanish was a symptom of that corruption, not a display bug. Fixed with a new `continuationChainStarts` helper that maps every physical line to the 1-based line starting its own continuation chain; `addComment` now pulls a mid-continuation target back to the chain's start before splicing, landing the new comment right before the whole multi-line entry instead of inside it. Lines outside any continuation are spliced in exactly as requested, unaffected.
  - **Task L55**: the add-row's "Line #" box showed the browser's default up/down spinner arrows - unwanted on what's meant to read as a small line-number badge to type into. Suppressed with `-moz-appearance: textfield` (Firefox) plus hiding the `::-webkit-outer-spin-button`/`::-webkit-inner-spin-button` pseudo-elements (Chrome/Edge/Safari), both scoped to the same `.field-order-row .comment-add-line-input` selector L51 already needed to reliably win the cascade. Typing digits and the field's own `min="1"` are unaffected.
  New coverage: `src/test/dspfWriter.test.js`'s "Task L54" scenario (built on a real engine-produced continuation, not hand-typed columns) and `src/test/dspfWebview.test.js`'s "Task L51"/"Task L55" checks (the width fix and both spinner-suppression rules, verified against the actual served stylesheet).

## 2026-09-05 — Task L53: "+ Fields from database file" now goes through click-to-place

- **0.10.42** — Asked directly: "+ Fields from database file" (Task L14) always dropped the whole checked batch immediately, stacked one row below the record's own current last field at a fixed column 2 - no way to choose where it lands, unlike "+ Field"/"+ Constant"/"Copy field", which all go through click-to-place. Reuses that exact same flow: a new `'DBFIELDS'` placement mode, armed by `beginDbFieldsPlacement` when "Add fields" is confirmed in the picker (closing the dialog without posting anything yet), consumed by a new `renderDbFieldsPlacementProps` panel once the canvas click lands - shows the source library/file, the field names, and an editable Start line/Column pre-filled from the click. `handleAddFieldsFromDatabase` in `extension.ts` now starts its per-field stacking from an optional `location` payload when given, falling back to the old default when it's not. The picker no longer needs a target record to open with either - the target is whichever record the placement click lands on, same as Copy/Paste's own cross-record case. New coverage in `src/test/dspfWebview.test.js` (the two-step click-to-place flow end to end) and `src/test/extension.test.js` (the `location` param directly).

## 2026-09-05 — Task P4: "Menu" toolbox tool — one click drops a ready-made PULLDOWN record

- **0.10.41** — Task P4 (LIMITATIONS-PLAN.md's P series): a `fabMenuBtn` ("Menu") item in the toolbox popover, right after Window. Simpler than Window turned out to be: a `PULLDOWN` record carries no `WINDOW` keyword and has no line/col/size of its own in real DDS (see `buildTypedRecordPlan`'s own `type === 'PULDWN'` case, which the "+ Add record" wizard already relies on) - a choice field elsewhere is what actually invokes a pulldown, at that field's own position, not the pulldown record's. So unlike Window, this deliberately does NOT arm a placement mode or wait for a canvas click at all - it commits immediately on click, closing the popover and creating the record in the same handler. Auto-names via a dedicated `nextPulldownRecordName()` (`PDN1`, `PDN2`, ...), matching Window's own "starts at 1, not 2" reasoning. Reuses `WebviewClientHelpers.placeRecordTemplate` (Task P2) for the actual insert. New test file `src/test/toolboxMenuTool.test.js`, covering the deliberate absence of any placement-mode/canvas-click step as well as the creation itself.

## 2026-09-05 — Task L52: modification tracking no longer corrupts multi-line/multi-fragment edits

- **0.10.40** — Reported directly with screenshots: with "Track modifications" on, editing a menu option's label text (spanning two separate CONSTANT fragments) into a longer piece of text made the field vanish from both the Options panel and the canvas. Root cause was in the one function both designers share for this, `applyModificationTracking` (`dspfWriter.js`): its diff loop pushed `comment(old[i])` then `tag(new[i])` per index, so whenever the changed region spanned more than one line on either side, the output interleaved comment/new-line pairs instead of grouping every comment ahead of every new line - and since DDS requires a continuation line to immediately follow the line it continues, an unrelated commented-out line landing in between corrupted the field outright. Split into two sequential passes (comment every changed/removed old line first, then append every changed/new line after) so a comment block and a new-content block never sandwich one another, regardless of line count on either side. Fixes both designers at once, since they share the function. New coverage in `src/test/dspfWriter.test.js`'s "Task L52" scenario, reproducing the exact reported case.

## 2026-09-05 — Task P3: "Window" toolbox tool — one click drops a ready-made window record

- **0.10.39** — Task P3 (LIMITATIONS-PLAN.md's P series): a `fabWindowBtn` ("Window") item in the toolbox popover arms a new `'WINDOW'` placement mode; one click on the canvas then commits IMMEDIATELY — unlike +Field/+Constant/+Add record, a window template has nothing left to ask (no name/length/type entry), so this skips the usual `pendingPlacement`/`renderPlacementProps` form entirely. Drops a new record named `WDW1`, `WDW2`, ... (a small dedicated counter starting at 1, not `DspfWriter.nextAvailableRecordName`'s own "always start at 2", which is built for copying an *existing* record) carrying `WINDOW(line col 10 40)` — line/col from the click, height/width defaulting to the same `10 40` the "+ Add record" wizard's own WINDOW type already falls back to — plus a `WDWTITLE('New window')` placeholder via the existing `setWindowTitleText` auto-quote/escape helper. Built on Task P2's `placeRecordTemplate` primitive — its first real exercise, per that task's own row. Also the popover's first item with no aside-panel original to proxy: its active-state toggle lives directly in `setPlacementMode` rather than via P1's `MutationObserver` mirroring, since there's nothing to mirror from. New coverage: `src/test/toolboxWindowTool.test.js` (arm/cancel placement mode, immediate commit with no intermediate form, geometry/title on the created record, WDW1→WDW2 auto-numbering, record-select switching to the new window).
...
- **0.10.37** — Task L51, two independent reports in the same message, both confirmed by direct reproduction rather than taken at face value:
  1. **The comment add-row's "Line #" box was still full row width, cramping the text input and "+" button** — reported directly with screenshots, even after L50 (0.10.35) narrowed `.comment-add-line-input` to `width: 30px`. Root cause: this stylesheet's generic `select, input[type=text], input[type=number] { width: 100%; ... }` rule has HIGHER CSS specificity (a type selector + an attribute selector) than a bare class selector, so it always won `width: 100%` — and every other overlapping property (background, color, border, padding, font-family, font-size) — over `.comment-add-line-input`, regardless of source order. Combined with the input's own `flex: 0 0 auto` (flex-basis: `auto`, so the browser falls back to `width` to size it), the losing `width: 30px` meant the box rendered at the row's full width the entire time — L50 shipped a correct-looking diff with zero actual visual effect. Fixed by rescoping the selector to `.field-order-row .comment-add-line-input` (two classes), which unconditionally outweighs the generic rule; verified via a jsdom `getComputedStyle` check that resolves to `100%` on the old selector and `30px` on the new one.
  2. **Newly-created fields with Usage `O` wrote a blank column 38 instead of the literal letter `O`** — reported directly. Per the DDS spec a blank column 38 is a valid synonym for Output, and `DspfParser`'s own `parseUsage(usageRaw) || 'O'` fallback already round-trips it correctly, so this wasn't a data-integrity bug — but written explicitly now per direct request: `serializePositionalCols` (`dspfWriter.js`) changed from `field.usage && field.usage !== 'O' ? field.usage : ' '` to `field.usage || 'O'`. Reading a pre-existing blank column 38 (from files iSDA wrote before this fix, or any other tool) is untouched and still parses as `'O'`.
  New coverage: `src/test/dspfWriter.test.js`'s "Task L51" scenario (5 checks: explicit write for both an explicit and an unset `usage`, round-trip, and legacy-blank-still-parses-as-O).
