...
4. **File-level attributes panel** - beyond command keys (done above),
   other file-level keywords (`DSPSIZ`, `REF`, `PRINT`, etc.) are parsed
   but still not shown/editable in either webview's sidebar.
5. **Sort elements** within a record (constants/fields reordering) - low
